...
def test_sort_dataframe_by_project_basic_ordering(known_projects):
    ...
    df = pd.DataFrame({
        'project_name': ['commons-utils', 'eqbench-es-default-1s', 'commons-utils-es-default-1s'],
    ...
    expected_order = ['eqbench-es-default-1s', 'commons-utils-es-default-1s', 'commons-utils']
...
def test_format_thousands_separator_basic():
    ...
    df = pd.DataFrame({
        'big_numbers': [1000, 50000, 1234567],
        'small_numbers': [100, 200, 300]
    })
...
def test_replace_project_names_with_macros():
    ...
    df = pd.DataFrame({
        'project_name': ['eqbench-es-default-1s', 'commons-utils', 'unknown-project'],
        'value': [1, 2, 3]
    })
...
def test_add_midrules_between_project_groups():
    ...
    df = pd.DataFrame({
        'project_name': ['eqbench-es-default-1s', 'eqbench-es-default-10s', 'commons-utils-es-default-1s', 'commons-utils']
    })
    ...
    expected = ['row1 \\\\', 'row2 \\\\', '\\midrule', 'row3 \\\\', '\\midrule', 'row4 \\\\']
...
def test_build_latex_table_content_empty_raises_error():
    ...
    with pytest.raises(ValueError, match="Cannot build LaTeX table from empty DataFrame"):
...
def test_build_latex_table_content_missing_project_column_with_midrules_raises_error():
    ...
    with pytest.raises(KeyError, match="Project column 'project_name' required for midrules"):

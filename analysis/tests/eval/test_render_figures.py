...
from teralizer.eval.artifacts import ArtifactId, RenderTarget
...
def test_materialize_writes_both_formats(tmp_path: Path):
    ...
    written = materialize(_report(), png_dir, pdf_dir, staging_root=tmp_path)
    assert (
        written.get(ArtifactId(RenderTarget.MARKDOWN, "bar")).path
        == png_dir / "bar.png"
    )
    assert (
        written.get(ArtifactId(RenderTarget.FIGURES, "bar")).path == pdf_dir / "bar.pdf"
    )
...
def test_pdf_is_keyed_by_figure_key_for_publishing(tmp_path: Path):
    ...
    assert {
        artifact.id.key for artifact in written.by_target(RenderTarget.FIGURES)
    } == {
        "first",
        "second",
    }
...
def test_both_formats_carry_provenance(tmp_path: Path):
    written = materialize(
        _report(), tmp_path / "png", tmp_path / "pdf", staging_root=tmp_path
    )
    pdf_bytes = written.get(ArtifactId(RenderTarget.FIGURES, "bar")).path.read_bytes()
    png_bytes = written.get(ArtifactId(RenderTarget.MARKDOWN, "bar")).path.read_bytes()
    assert b"/Subject" in pdf_bytes
    assert b"teralizer.eval example @ " in pdf_bytes
    assert b"teralizer.eval example @ " in png_bytes
...
def test_pdf_is_vector_with_embedded_fonts(tmp_path: Path):
    ...
    pdf = written.get(ArtifactId(RenderTarget.FIGURES, "bar")).path
    assert b"/FontFile2" in pdf.read_bytes()


def test_directories_are_created(tmp_path: Path):
    target = tmp_path / "absent" / "deeper"
    materialize(_report(), target / "png", target / "pdf", staging_root=tmp_path)
...
def test_a_report_without_figures_writes_nothing(tmp_path: Path):
    ...
    written = materialize(
        report, tmp_path / "png", tmp_path / "pdf", staging_root=tmp_path
    )

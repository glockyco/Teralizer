...
def test_teralizer_projects_with_runtime(dev_conn, known_projects):
    ...
    assert returned_projects.issubset(known_projects_set), (
        f"Unknown projects: {returned_projects - known_projects_set}"
    )
...
def test_mutation_results_by_project_variant(dev_conn, basic_variants):
    ...
    assert returned_variants == expected_variants, (
        f"Expected {expected_variants}, got {returned_variants}"
    )
...
def test_exclusions_all(dev_conn, known_variants):
    ...
    assert returned_variants.issubset(known_variants_set), (
        f"Unknown variants: {returned_variants - known_variants_set}"
    )
...
def test_runtime_comparison_by_variant(dev_conn, generalization_variants):
    ...
    assert returned_variants.issubset(generalization_variants_set), (
        f"Non-generalization variants found: {returned_variants - generalization_variants_set}"
    )

...
from .exports import (
    ...
    get_dataset_macro, get_variant_macro
)
...
def sort_dataframe_by_project(df: pd.DataFrame, project_column: str = 'project_name', 
                             variant_column: str = 'variant') -> pd.DataFrame:
    ...
    def sort_key(idx):
        ...
        variant = df.loc[idx, variant_column] if variant_column in df.columns else 'INITIAL'
...
def sort_dataframe_by_variant(df: pd.DataFrame, variant_column: str = 'variant') -> pd.DataFrame:
    ...
    variant_order = {
        'ORIGINAL': 0, 'INITIAL': 1, 'SHARED': 2, 'BASELINE': 3,
        'NAIVE_10_TRIES': 4, 'NAIVE_50_TRIES': 5, 'NAIVE_200_TRIES': 6,
        'IMPROVED_10_TRIES': 7, 'IMPROVED_50_TRIES': 8, 'IMPROVED_200_TRIES': 9
    }
...
def apply_standard_ordering(df: pd.DataFrame, project_column: str = 'project_name', 
                          variant_column: str = 'variant') -> pd.DataFrame:
...
def format_percentage_columns(df: pd.DataFrame, columns: List[str], 
...
def standardize_column_names(df: pd.DataFrame, mapping_dict: Dict[str, str]) -> pd.DataFrame:
...
def replace_project_names_with_macros(df: pd.DataFrame, 
                                     project_column: str = 'project_name') -> pd.DataFrame:
    ...
    known_projects = {
        'eqbench-es-default-1s', 'eqbench-es-default-10s', 'eqbench-es-default-60s',
        'commons-utils-es-default-1s', 'commons-utils-es-default-10s', 'commons-utils-es-default-60s',
...
def replace_variant_names_with_macros(df: pd.DataFrame, 
                                    variant_column: str = 'variant') -> pd.DataFrame:
    ...
    known_variants = {
        'ORIGINAL', 'INITIAL', 'SHARED', 'BASELINE',
...
def add_midrules_between_project_groups(table_rows: List[str], 
                                       df: pd.DataFrame,
                                       project_column: str = 'project_name') -> List[str]:
    ...
    if len(table_rows) != len(df):
        raise ValueError(f"Length mismatch: {len(table_rows)} table rows vs {len(df)} DataFrame rows")
...
def format_table_with_macros(df: pd.DataFrame, 

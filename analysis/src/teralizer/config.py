...
class DatabaseConfig:
    ...
    def _validate_connection(self, engine, database):
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                if missing_objects:
                    missing_str = "\n".join([f"  - {obj_type}: {name}" for obj_type, name in missing_objects])
                    ...
                    for filename, file_objects in sql_files.items():
                        ...
                        if file_has_missing:
                            warning = ""
                            if filename == 'create-tables.sql' and 'tables' in file_objects and file_objects['tables']:
                                warning = " (WARNING: This will DROP all existing tables and data!)"
                            scripts_needed.append(f"  - src/main/resources/db/{filename}{warning}")
    ...
    def _parse_sql_files(self):
        ...
        patterns = {
            ...
            'views': re.compile(r'CREATE\s+VIEW\s+(?:IF\s+NOT\s+EXISTS\s+)?(\w+)', re.IGNORECASE),
            'materialized_views': re.compile(r'CREATE\s+MATERIALIZED\s+VIEW\s+(?:IF\s+NOT\s+EXISTS\s+)?(\w+)', re.IGNORECASE),
            'functions': re.compile(r'CREATE\s+(?:OR\s+REPLACE\s+)?FUNCTION\s+(\w+)', re.IGNORECASE)
        }

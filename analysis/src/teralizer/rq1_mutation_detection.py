...
def compute_mutator_statistics(variant_data: pd.DataFrame, 
                             project_data: pd.DataFrame) -> pd.DataFrame:
    ...
    min_max_percent = project_data.groupby('mutator')['project_percent'].agg(
        ['min', 'max']
    ).reset_index()
...
def compute_project_mutation_coverage(df_mutations: pd.DataFrame, 
                                    df_total_classes: pd.DataFrame) -> pd.DataFrame:
...
def compute_detection_improvements(df: pd.DataFrame, 
                                 baseline_variant: str = 'INITIAL') -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        if row['variant'] == baseline_variant:
            ...
            if project in baseline_data.index:
                baseline_detected = baseline_data.loc[project, 'detected_of_covered_pct']
                abs_improvement = row['detected_of_covered_pct'] - baseline_detected
                rel_improvement = (abs_improvement / baseline_detected * 100) if baseline_detected != 0 else 0.0
            else:
                abs_improvement = 0.0
                rel_improvement = 0.0
...
def generate_detections_per_mutator_table(df: pd.DataFrame) -> str:
    ...
    df_table['mutator'] = df_table['mutator'].str.replace('RemoveConditional_ORDER_ELSE', 'RemoveConditionalOrderElse')
    df_table['mutator'] = df_table['mutator'].str.replace('RemoveConditional_EQUAL_ELSE', 'RemoveConditionalEqualElse')
    
    # Format data for table
    df_table['total'] = df_table['total_mutants'].astype(int)
    
    # Format percentages
    percent_cols = ['percent', 'min_percent', 'max_percent', 
                   'INITIAL', 'NAIVE_200_TRIES', 'IMPROVED_200_TRIES']
...
def generate_mutants_per_project_table(df: pd.DataFrame) -> str:
    ...
    df_table['test_inclusion_pct'] = (df_table['included_tests'] / df_table['total_tests'] * 100).round(1)
    df_table['class_inclusion_pct'] = (df_table['included_classes'] / df_table['total_classes'] * 100).round(1)
    ...
    # Create DataFrame for build_latex_table_content
    rows_df = pd.DataFrame([row.split(' & ') for row in table_rows])
    rows_df.columns = ['Project', 'Test Methods', 'Impl. Classes', 'Total', 'Covered', 'Uncovered']
...
def generate_mutation_detection_comparison_table(df: pd.DataFrame) -> str:
    ...
    # Create DataFrame for build_latex_table_content
    rows_df = pd.DataFrame([row.split(' & ') for row in table_rows])
    rows_df.columns = ['Project', 'Detected', 'Mutants', 'AvgOps', 'MedianOps', 'AvgConstraints', 'MedianConstraints', 'AvgUsed', 'MedianUsed']
...
def generate_detections_per_mutator_csv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['mutator'] = df['mutator'].str.replace('RemoveConditional_ORDER_ELSE', 'RemoveConditionalOrderElse')
    df['mutator'] = df['mutator'].str.replace('RemoveConditional_EQUAL_ELSE', 'RemoveConditionalEqualElse')
    ...
    for _, row in df.iterrows():
        csv_data.append({
            ...
            'initial_detection_rate': format_detection_rate_decimal(row.get('INITIAL', 0)),
            'naive_200_detection_rate': format_detection_rate_decimal(row.get('NAIVE_200_TRIES', 0)),
            'naive_200_improvement': row.get('detected_diff_naive', 0.0),
            'improved_200_detection_rate': format_detection_rate_decimal(row.get('IMPROVED_200_TRIES', 0)),
...
def generate_mutation_detection_figure_csv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        csv_data.append({
            ...
            'detected_improvement_absolute': float(row['absolute_improvement']) if pd.notna(row['absolute_improvement']) else 0.0,
            'detected_improvement_relative': float(row['relative_improvement']) if pd.notna(row['relative_improvement']) else 0.0,
            'baseline_initial_detected': float(row['detected_of_covered_pct']) if row['variant'] == 'INITIAL' else 0.0
        })

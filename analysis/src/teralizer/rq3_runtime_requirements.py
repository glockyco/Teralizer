...
def get_evosuite_vs_teralizer_efficiency(
...
def get_detailed_efficiency_comparison(
    conn, variant: str = "IMPROVED_200_TRIES"
) -> pd.DataFrame:
...
def compute_pareto_efficiency_analysis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prefix_budget = df["project_name"].apply(
    ...
    for project_prefix in project_prefixes:
        ...
        evosuite_points = (
            proj_df.groupby(["project_prefix", "search_budget"]).first().reset_index()
        )
        ...
        es_points.rename(
            columns={
                ...
                "evosuite_detected": "detection_rate",
            },
            inplace=True,
        )

        # Teralizer variants (EvoSuite + Teralizer)
        naive_points = proj_df[
            proj_df["teralizer_variant"].str.startswith("NAIVE", na=False)
        ][
            [
                "project_prefix",
                "search_budget",
        ...
        improved_points = proj_df[
            proj_df["teralizer_variant"].str.startswith("IMPROVED", na=False)
        ][
            [
                "project_prefix",
                "search_budget",
...
def generate_pareto_figure(df_pareto: pd.DataFrame, project_name: str) -> plt.Figure:
    ...
    ax.scatter(
        es_points["runtime_seconds"],
        es_points["detection_rate"],
        marker="o",
        color="blue",
        alpha=0.3,
        s=40,
    ...
    ax.scatter(
        naive_points["runtime_seconds"],
        naive_points["detection_rate"],
        marker="x",
        color="red",
        alpha=0.3,
        s=40,
    ...
    ax.scatter(
        improved_points["runtime_seconds"],
        improved_points["detection_rate"],
        marker="^",
        color="green",
        alpha=0.3,
        s=40,
    ...
    ax.plot(
        pareto_points["runtime_seconds"],
        pareto_points["detection_rate"],
        linestyle="--",
        color="black",
        linewidth=1.2,
        zorder=2,
...
def generate_pareto_points_table(df_pareto: pd.DataFrame, project_name: str) -> str:
    ...
    project_data = df_pareto[
        (df_pareto["project_name"] == project_name) & (df_pareto["is_pareto_optimal"])
    ].copy()
        ...
        csv_data.append(
            {
                ...
                "teralizer_variant": get_variant_macro(row["teralizer_variant"])
                if row["teralizer_variant"] != "ES_ONLY"
                else "ES_ONLY",
                "detection_rate_decimal": format_detection_rate_decimal(
                    row["detection_rate"]
                ),

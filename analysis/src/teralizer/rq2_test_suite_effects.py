...
from .exports import (
    get_variant_macro, standardize_project_name
)
...
def get_test_count_changes_by_project_variant(conn, variants: List[str] = None) -> pd.DataFrame:
    ...
    return df[['project_name', 'a_variant', 'b_variant', 'tests_before', 'added_tests', 
...
def get_line_count_changes_by_project_variant(conn, variants: List[str] = None) -> pd.DataFrame:
    ...
    """
    df = get_generalization_effects_data(conn, variants)
    return df[['project_name', 'a_variant', 'b_variant', 'lines_before', 'added_lines',
...
def get_runtime_changes_by_project_variant(conn, variants: List[str] = None) -> pd.DataFrame:
    ...
    """
    df = get_generalization_effects_data(conn, variants)
    return df[['project_name', 'a_variant', 'b_variant', 'runtime_before', 'added_runtime',
               'removed_runtime', 'runtime_after', 'runtime_delta', 'runtime_delta_pct']]
...
def compute_test_suite_change_statistics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_cols = ['tests_before', 'added_tests', 'removed_tests', 'tests_after', 'tests_delta']
    for col in numeric_cols:
        df_sorted[col] = pd.to_numeric(df_sorted[col], errors='coerce').fillna(0).astype(int)
    
    df_sorted['tests_delta_pct'] = pd.to_numeric(df_sorted['tests_delta_pct'], errors='coerce').fillna(0.0)
...
def compute_line_count_change_statistics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Convert numeric columns to proper types
    numeric_cols = ['lines_before', 'added_lines', 'removed_lines', 'lines_after', 'lines_delta']
    for col in numeric_cols:
        df_sorted[col] = pd.to_numeric(df_sorted[col], errors='coerce').fillna(0).astype(int)
    
    df_sorted['lines_delta_pct'] = pd.to_numeric(df_sorted['lines_delta_pct'], errors='coerce').fillna(0.0)
...
def compute_runtime_change_statistics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Convert numeric columns to proper types
    numeric_cols = ['runtime_before', 'added_runtime', 'removed_runtime', 'runtime_after', 'runtime_delta']
    for col in numeric_cols:
        df_sorted[col] = pd.to_numeric(df_sorted[col], errors='coerce').fillna(0.0)
    
    df_sorted['runtime_delta_pct'] = pd.to_numeric(df_sorted['runtime_delta_pct'], errors='coerce').fillna(0.0)
...
def compute_test_vs_generalization_runtime_statistics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Convert numeric columns to proper types
    numeric_cols = ['mean_t_runtime_ms', 'mean_g_runtime_ms', 'mean_runtime_diff_ms', 
                   'ratio_of_mean_runtimes', 'mean_runtime_diff_per_try_ms']
    for col in numeric_cols:
...
def generate_tests_per_project_table(df: pd.DataFrame) -> str:
    ...
    rows_df.columns = ['Project', 'Variant', 'Before', 'Added', 'Removed', 'After', 'Delta', 'DeltaPct']
...
def generate_test_runtime_differences_csv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        csv_data.append({
            ...
            'runtime_ratio_generalization_to_test': float(row['ratio_of_mean_runtimes']),
            'tries_count': int(row['tries']),
            'mean_runtime_difference_per_try_ms': float(row['mean_runtime_diff_per_try_ms'])
        })

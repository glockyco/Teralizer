...
public class ModelToZ3Transformer extends ModelVisitor {
    ...
    public Expr<?> transform(boolean value) {
        return this.context.mkBool(value);
    }
    ...
    public Expr<?> transform(teralizer.domain.Model model) {
        if (model == null) {
        ...
        switch (operation.op) {
            ...
            case ATAN2:
                expr = this.context.mkApp(
                    this.context.mkFuncDecl(
                        operation.op.toString(),
                        new Sort[]{this.context.mkRealSort(), this.context.mkRealSort()},
                        this.context.mkRealSort()
                    ),
                    left,
                    right

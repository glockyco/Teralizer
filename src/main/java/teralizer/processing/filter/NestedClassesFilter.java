...
public class NestedClassesFilter extends AbstractFilter {
    ...
    public FilterResult check() throws Exception {
        ...
        // A class that is absent from the model gives no evidence about nested classes. This
        // filter only reports, so it accepts. A failure here would stop every other filter.
        if (testClass == null) {

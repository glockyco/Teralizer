...
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_detection_comparison;
DROP MATERIALIZED VIEW IF EXISTS mv_efficiency_comparison_evosuite_vs_teralizer;
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_results_by_project_variant_mutator;
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_results_by_project_variant;
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_results_by_variant_mutator;
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_results_by_variant;
DROP MATERIALIZED VIEW IF EXISTS mv_generalization_effects;
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_status_changes;
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_variant_comparison;
DROP MATERIALIZED VIEW IF EXISTS mv_pit_mutation_report_extension;
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_covering_generalizations;
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_covering_assertions;
DROP MATERIALIZED VIEW IF EXISTS mv_mutation_covering_tests;
DROP MATERIALIZED VIEW IF EXISTS mv_pit_mutation_coverage;
DROP MATERIALIZED VIEW IF EXISTS mv_pit_mutation_report;
DROP MATERIALIZED VIEW IF EXISTS mv_pit_coverage_report;
DROP MATERIALIZED VIEW IF EXISTS mv_pit_mutation_report_location;
DROP MATERIALIZED VIEW IF EXISTS mv_pit_coverage_report_location;
DROP MATERIALIZED VIEW IF EXISTS mv_pit_location;
...
BEGIN
    ...
    stage_order := CASE
        ...
        WHEN stage = 'FILTER_TESTS_ORIGINAL' THEN 10

        WHEN stage = 'ANALYZE_TESTS' THEN 11
        WHEN stage = 'FILTER_TESTS' THEN 12
        WHEN stage = 'FILTER_ASSERTIONS' THEN 13

        WHEN stage = 'ADD_JPF_INSTRUMENTATION' THEN 14
        WHEN stage = 'BUILD_PROJECT_INSTRUMENTED' THEN 15
        WHEN stage = 'EXECUTE_JPF' THEN 16
        WHEN stage = 'ANALYZE_JPF' THEN 17
        WHEN stage = 'CLEANUP_JPF_INSTRUMENTATION' THEN 18

        WHEN stage = 'BUILD_PROJECT_INITIAL' THEN 19
        WHEN stage = 'EXECUTE_TESTS_INITIAL' THEN 20
        WHEN stage = 'COLLECT_JUNIT_REPORTS_INITIAL' THEN 21

        WHEN stage = 'CLEANUP_GENERALIZATION' THEN 22
        WHEN stage = 'GENERALIZE_TESTS' THEN 23
        WHEN stage = 'BUILD_PROJECT_GENERALIZED' THEN 24
        WHEN stage = 'EXECUTE_TESTS_GENERALIZED' THEN 25
        WHEN stage = 'COLLECT_JUNIT_REPORTS_GENERALIZED' THEN 26
        WHEN stage = 'FILTER_GENERALIZATIONS' THEN 27

        WHEN stage = 'RESTORE_ORIGINAL_BUILD' THEN 28
        WHEN stage = 'COLLECT_JACOCO_DATA_ORIGINAL' THEN 29
        WHEN stage = 'COLLECT_PIT_DATA_ORIGINAL' THEN 30
        WHEN stage = 'COLLECT_JACOCO_DATA_INITIAL' THEN 31
        WHEN stage = 'COLLECT_PIT_DATA_INITIAL' THEN 32
        WHEN stage = 'RESTORE_GENERALIZED_BUILD' THEN 33
        WHEN stage = 'COLLECT_JACOCO_DATA_GENERALIZED' THEN 34
        WHEN stage = 'COLLECT_PIT_DATA_GENERALIZED' THEN 35
    END;
    ...
    SELECT
        ...
        CASE
            WHEN t.stage IN ('SETUP_PROJECT', 'ADD_DEPENDENCIES', 'BUILD_PROJECT_ORIGINAL',
                            ...
                            'FILTER_TESTS_ORIGINAL', 'ANALYZE_TESTS', 'FILTER_TESTS',
                            'FILTER_ASSERTIONS')
            THEN 'Stage 1 + 2'
            ...
            THEN 'Stage 4'
            WHEN t.stage IN ('RESTORE_ORIGINAL_BUILD', 'COLLECT_JACOCO_DATA_ORIGINAL',
                            'COLLECT_PIT_DATA_ORIGINAL', 'BUILD_PROJECT_INITIAL',
                            'EXECUTE_TESTS_INITIAL', 'COLLECT_JUNIT_REPORTS_INITIAL',
                            'COLLECT_JACOCO_DATA_INITIAL', 'COLLECT_PIT_DATA_INITIAL',

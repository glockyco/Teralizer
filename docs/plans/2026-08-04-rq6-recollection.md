...
- [x] Launch the fixed code into a fresh `postgres_reporeapers_rq6_v3` database, preserving
      the superseded `postgres_reporeapers_rq6` and `postgres_reporeapers_rq6_v2` databases
      until the new measurement is signed off. Store data under `data/reporeapers-rerun-v3`,
      keep the 14400 s project ceiling, and use the process name `reporeapers-rq6-v3`. Do not
...
- [ ] Compare the funnel against the previous measurement and account for every band that
      moved.
  Verification: `uv run --directory analysis python -m teralizer.eval rq6 --db postgres_reporeapers_rq6_v3 --targets md`
...
- [ ] Regenerate every RQ6 artifact plus the dataset report against the new database.
      `rq6_causes.DEFAULT_DB` already names `postgres_reporeapers_rq6_v3`; the dataset

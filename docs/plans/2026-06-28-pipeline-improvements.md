...
- [ ] D-1 · Tag concretized symbolic terms so incomplete specs are explicit, not silent narrowing — when a value is already concrete (raw bits under `z3`), the listener bakes it in as a constant, which is how the maxUlps spec collapsed to `0 < maxUlps`. Deferred to the `2026-06-28-maxulps-raw-bits-lane` plan: concretization tagging only matters once raw-bits function expressions are mapped to a Model node (Gap 2) and the per-probe SPF config (D-3/B-4) makes raw bits symbolic; without those, tagging would flag every raw-bits probe as incomplete, which is already documented as the lane's Gap 1.

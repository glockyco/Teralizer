...
### Task 11: Corpus verification on the 23-project spike (operator-assisted)

Prove: recall rose, tiers are honest, the completing corpus did not regress, and the extraction/validation cost is known.

**Files:** none (disposable DB). Record results in `docs/plans/2026-06-28-mut-id-targeting-and-coverage.md`.

- [x] **Step 1: Rerun the 23-project spike** with the current branch into `postgres_fusion_spike`. The rerun that satisfies this step is `2026-07-03-generalized-validation-repair` Task 5; the validation-layer fixes there are a precondition for Step 5's seed-kill data.
- [x] **Step 2: Tier funnel.** Run `uv run --directory analysis python -m teralizer.mut_resolution_funnel` against the spike DB. Record: per-tier shares; `MissingValue`×status cross-tab. Acceptance: `MissingValue` rejects with `status='RESOLVED'` ≈ 0 (a resolved pick must populate `tested_*`); the pre-fusion `MissingValue` count for the same projects strictly exceeds the post-fusion count.
- [x] **Step 3: No-regression census.** Compare per-variant `IMPROVED_100_TRIES` generalization counts per project against the pre-fusion baseline matched by `project.root_path`. Acceptance: every project completing both runs keeps or gains generalizations; original-suite timeout exclusions and zero-generation projects are recorded separately.
- [x] **Step 4: Mis-targeting spot check, stratified.** Sample manually: 10 T1, 5 T2, 10 T3, 20 T4 newly-resolved assertions (`ORDER BY random()` with a fixed seed via `setseed`). Read each test's source; record whether the pick is the developer-intended method. Acceptance: T1 = 100% intended (any miss is a resolver bug — fix and re-run); T3/T4 rates recorded in the audit doc (no threshold — they are the honesty story and the He-et-al. comparison point).
- [x] **Step 5: Compute cost + seed-kill share.** From the `task`, `generalization`, and `jqwik_property_execution` tables, record `ANALYZE_TESTS`/JPF/setup runtime totals, raw seed-kill share, and the newly-attempted seed-kill share keyed across DBs by `(project.root_path, test.test_method_qualified_name, assertion_source_code, occurrence ordinal)`. `EXECUTE_TESTS_*` runtimes are only comparable within the post-fork-fix binary.
- [x] **Step 6: Topology distribution (the R1/R2 decision gate).** From the funnel's shape × provenance cross-tab, record: (a) the realized R1 opportunity — `CHAINED_*`/`OPERATOR_COMPOSITE`/`CTOR_ONLY` rows with input sites; (b) the R2 sub-family — `SINGLE_CALL` × `LOCAL_CTOR`/`LOCAL_CTOR_MUTATED` counts. Per `2026-07-02-input-topology-spike` §R2: >5k clean `LOCAL_CTOR`-rooted zero-argument inspectors ⇒ design R2 properly; else T3 stays out of scope.
- [x] **Step 7: Record everything** in `2026-06-28-mut-id-targeting-and-coverage` (tables: tier shares, MV delta, spot-check rates, cost delta, seed-kill share, validation coverage, exclusion accounting, topology distribution) and check this task's boxes.
...
- [x] **Step 1:** Verify spec acceptance criteria (`2026-07-02-mut-id-confidence-fusion` §Acceptance) each map to a completed task. Code-side criteria (totality, observation rows, grade separation, characterization tests) are satisfied by Tasks 1-10; empirical criteria (tier funnel counts, census non-regression, mis-targeting spot check, cost + seed-kill share, topology distribution) are satisfied by Task 11.
- [x] **Step 2:** Set this plan's `status: implemented` is **not** done by hand — run `omp-plans complete 2026-07-02-static-mut-id-fusion` (moves to archive, regenerates INDEX, validates).
- [x] **Step 3:** Commit any doc deltas: `git commit -am "docs(plans): record MUT-id fusion spike results"`.

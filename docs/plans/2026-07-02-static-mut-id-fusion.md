...
- [ ] **Step 1: Rerun the 20-project spike** with the current branch into a fresh disposable DB (operator provides the config; `./gradlew run -Dteralizer.config=<spike-config>`). The rerun that satisfies this step is `2026-07-03-generalized-validation-repair` Task 5 — the validation-layer fixes there are a precondition for Step 5's seed-kill data (without them, `jqwik_property_execution` covers only 2/19 projects and the metric is not computable). Steps 2, 3, 4, and 6 are extraction-side and may run against the existing spike DB before that rerun; re-verify census stability afterwards.
...
- [x] **Step 3: No-regression census.** Compare generalization counts per project against the pre-fusion baseline for the same 20 projects (operator has the baseline; the ~250-generalization census is the reference). Acceptance: no project loses generalizations. Any loss ⇒ diff the picks for that project's assertions (`resolved_method_name` vs the old `tested_method_name`) — a changed pick on a previously-working assertion is a contract-1 violation; fix before proceeding. Divergences (a)/(b) from the behavior contract are the only sanctioned pick changes.
...
- [ ] **Step 5: Compute cost + seed-kill share.** From the `task` table, compare total wall-clock of the JPF block (stages `EXECUTE_JPF`/`ANALYZE_JPF`) before vs after fusion for the same projects; record the delta alongside the newly-attempted assertion count (previously-`MissingValue` assertions now entering JPF, from Step 2). `EXECUTE_TESTS_*` runtimes are only comparable within the post-fork-fix binary. Additionally record the **seed-kill share**: among newly-attempted generalizations, the fraction of `jqwik_property_execution` rows with `tries = 1 AND diagnostic_kind = 'ASSERTION_FAILED'` (first-trial failure = incoherent pick caught by the coherence backstop — the direct empirical test of the spec's bounded-downside claim, and the decision input for the early coherence gate, `2026-07-02-recipe-seam-review`). Report it with its denominator: `jqwik_property_execution` coverage (gens-with-rows / total gens; projects-with-rows / projects-with-gens) plus the exclusion table (per project: build failure / execution timeout / collected) from `2026-07-03-generalized-validation-repair` Task 5 — validation coverage is structurally incomplete (uniform 60s execution budget; binding decision there), so the seed-kill share is always qualified by which slice of the corpus it was measured on.

...
- **Domain Model** (`Expression` tree; compile-strict `ModelFolder`, partial-by-intent `ModelVisitor`): total, small, a missing node kind is a build break.
- **Recipe seam** (single derivation, persisted JSON, resolve-on-write): killed the 4× drift surface; made R1 a payload change.
...
- **Resolver prose**: heuristics are the domain; the tier/signal/corroborator vocabulary is the abstraction. Do not force it into a strategy-pattern zoo.
...
3. **The R1 dual path.** `expressionRecipe ? … : …` branch pairs in both big tasks: two wrapper builders, two call-site builders, two site-rewrite blocks. The invocation recipe is a degenerate expression recipe (oracle expression = the call; sites = argument positions).
...
- `MethodUnderTestResolver.isThisOrUnqualified`: `"this".equals(target.toString())` — AST comparison by pretty-print; should be `instanceof CtThisAccess`.
- `MethodUnderTestResolver.sameField`: simple-name-only comparison; hierarchy shadowing collides (guarded by the this-receiver check, latent).
...
**Tier A — structural (spec → plan → fixture-gated tasks; sequence A3 → A1 → A2 → A4):**
- A1 Unify on expression recipes: invocation recipe becomes an expression recipe with argument-position sites. Deletes both index-keyed rewrite blocks, the second wrapper builder, the `(CtInvocation)` casts, the dual sentinel semantics. `_target_`/lifted-locals generalize to site kinds. Effort M–L.
- A2 Split mega-tasks into Task (orchestrate) + Builder (pure codegen: recipe in, `CtClass` out, no DB/IO). Halves both files; codegen becomes unit-testable. Effort M.
- A3 Replace sentinel ints with a shared `InputKind` (+ `EXPRESSION_SITE`). Effort S–M.
- A4 Extract `FocalTypeResolver` + `InputTopologyClassifier` from the resolver; caches move into `TaskContext`. Resolver ≈800 lines of pure resolution. Effort S–M.

A1+A2 subsume most of the old review's C-1 (single emitter); after them C-1 shrinks to a leftover check.

**Tier B — hygiene (one batch):** snippet rule (snippets only for leaf identifier references; structured code is built as nodes — fix the invocation-path wrapper body), rename `jpf.Invocation` → `CapturedInvocation`, `TypeFilter` sweep, `isThisOrUnqualified`/`sameField` fixes, move `WideningLicense` to a policy package, table-ify `TestAnalysis` indices and the annotation rewrite, delete decided TODOs.

**Tier C — comprehension docs:** `docs/artifacts.md` artifact gallery (observed artifacts, annotated); architecture.md codegen-technology section + snippet rule; complete per-package responsibilities.

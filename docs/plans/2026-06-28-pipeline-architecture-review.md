...
- **Gap 1 — config:** enable `symbolic.fp=true`+`symbolic.dp=z3bitvector`+`bvlength=64` for raw-bits MUTs. → done (D-3 + B-4): auto-selected by detection. `z3`+`fp` is upstream-broken (jpf-symbc#35), so bitvector is mandatory. B-3 (per-variable width) remains for general soundness.
...
- §5.3.2 effectiveness ("extended constraint encoding to reduce filter-and-regenerate"): C-3 (consumed-clause telemetry, done) is the measurement seam; C-4 (by-construction recipes) is the encoding seam, but its only evidence-backed member is the ulps recipe in the raw-bits lane (Gap 3) — further recipes are telemetry-gated. Our affine work already moved past Algorithm 1.
- §5.3.3 efficiency: C-1/C-6 reduce codegen drift and per-test overhead; the ulps by-construction recipe (raw-bits lane Gap 3) eliminates filter-miss exhaustion for the one shape that cannot be filtered.
...
3. ~~Effectiveness: C-3 consumed-clause telemetry~~ (done). C-4 is no longer a standalone step — its recipe-library infrastructure + only evidence-backed recipe (ulps) build inside the raw-bits lane (Gap 3); further recipes are gated on generation-coverage shape telemetry.
4. **Next — per-probe config: ~~B-4 + D-3~~ (done, auto-selected by detection); B-3 (per-variable bit-vector width) remains for general raw-bits soundness.**
5. Raw-bits lane (L): A-5 + A-3 node (Gap 2), then recipe-library + ulps recipe (Gap 3).

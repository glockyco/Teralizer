...
4. **By-construction coverage is partially populated but not surfaced.** Per-parameter `consumedClauseIds` are populated by `NumericDomainPlanner` (`4f680d7f`), but plan-level aggregation and the generation-coverage telemetry surface are not wired, so by-construction coverage cannot be reported. The residual filter stays unconditional (residual-only filtering is a non-goal per the clause-driven spec). (C-3)
...
- **C-3 · `consumedClauseIds` is populated per-parameter but not aggregated or surfaced.** `NumericDomainPlanner` now reports consumed clause ids per parameter (`4f680d7f`), but `InputGenerationPlan` does not aggregate them to plan level and the generation-coverage telemetry does not consume them. The residual filter stays unconditional (residual-only filtering is a non-goal per the clause-driven spec — no outcome change, only added unsoundness surface). Opportunity: aggregate per-parameter consumed ids to plan level and wire them to the generation-coverage telemetry. Effort M · Sound? n/a (telemetry only; filter unchanged).
...
3. Effectiveness (M–L): C-3 consumed-clause telemetry, then C-4 by-construction recipe library.

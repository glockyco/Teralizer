...
1. **Silent-failure surfaces everywhere.** `ModelVisitor` hooks default to no-ops; `Operator.get()` is assert-only; `VariableConstraintExtractor` has a `do-nothing` default; `SpfToModelTransformer` throws `UnsupportedOperationException` on three node kinds; the Error JSON serializer/deserializer field names disagree. Failures surface as crashes or silently-wrong specs rather than typed, attributable outcomes. (A-1, A-2, A-5, D-2)
...
4. **Generation soundness rests on the residual filter; by-construction coverage is shallow and untracked.** `consumedClauseIds` is plumbed but never populated, so the full `filter(inputJava)` is always emitted and `TooManyFilterMisses` exclusions are the dominant failure mode the paper reports. (C-3)
...
- **C-2 · Boolean constraints are discarded; char only recently encoded.** `NumericDomainPlanner.supports` covers INTEGER/REAL/CHAR; BOOLEAN falls to `defaultRecipe` → `Arbitraries.of(true,false)` with no constraint encoding, and `VariableConstraintExtractor` never extracts boolean constraints upstream anyway. Opportunity: a `BooleanDomainPlanner` (equality/just) + boolean extraction. Effort S–M · Sound? no (filter still guards; just lost diversity/boundary precision).
- **C-3 · `consumedClauseIds` is plumbed but never populated → residual-only filtering impossible.** `ParameterGenerationPlan`/`InputGenerationPlan` carry the field, but `NumericDomainPlanner` and `defaultRecipe` always pass `Collections.emptySet()`, so `residual = all clauses` and the full `filter(inputJava)` is always emitted. This is the seam for the paper's §5.3.2 "extended constraint encoding to reduce filter-and-regenerate cycles." Opportunity: have each recipe report the exact clause ids it enforces by construction; emit a residual-only filter. Effort M · Sound? yes if done carelessly — must only drop clauses provably enforced; default to keeping.
...
- §5.3.2 effectiveness ("extended constraint encoding to reduce filter-and-regenerate"): C-3 (residual-only filtering) + C-4 (by-construction recipes) are the exact seams; our affine work already moved past Algorithm 1.
- §5.3.3 efficiency: C-1/C-6 reduce codegen drift and per-test overhead; C-3 directly attacks the filter-miss cost the paper measures.
...
3. Effectiveness (M–L): C-3 residual-only filtering, then C-4 by-construction recipe library.

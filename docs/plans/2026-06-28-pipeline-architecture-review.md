...
5. **Latent correctness bugs:** the `MinMax` double lower-bound bug (B-2), the `LCMP` long→int truncation (B-5, fork commit `146a4a0`), the Error JSON round-trip (A-2), unquoted `ConstantString` (A-4), `Operator.get()` assert-only (D-2), and `RealConstraints` NaN-unguarded bounds (C-5) are fixed.
...
- **B-5 · ~~`LCMP` truncates long to int in one branch.~~** → fixed in the fork (`146a4a0 fix(symbc): preserve long width in LCMP comparison`; verified at `LCMP.java:57-58` — both operands `popLong()`). `toIntExact`'s missing overflow path (the symcrete `LCMP`/`DCMP` control-flow decoupling) remains a separate, open concern.
...
- **B-7 · Fork hygiene: no upstream remote; solver settings are dead weight in collect mode.** The vendored fork (`glockyco/jpf-symbc` @ `gradle-build`, nested `glockyco/jpf-core` @ `java-8`) has no `upstream` remote configured — divergence from upstream jpf-symbc is not diffable without adding one; ~30 fork commits carry the string/raw-bits/LCMP work. Load-bearing negative finding: **in collect-constraints mode no solver is ever invoked** — `PathCondition.solve()`/`simplify()` return `true` immediately when `SymbolicInstructionFactory.collect_constraints` is set (`PathCondition.java:325-340`), and `symbolic.string_dp` defaults to `"none"` (`SymbolicInstructionFactory.java:652-660`) which makes string satisfiability checks vacuously true. This is *correct* for Teralizer (the concrete path is the satisfiability witness; constraints are only collected, never solved at extraction time) — so do NOT "fix" the missing `string_dp`, and treat solver-bridge findings (B-3) as relevant only to lanes that actually solve (raw-bits scorecard). Opportunity: add the upstream remote + document the fork-commit inventory. Effort S · Sound? no.
...
- **C-5 · ~~`RealConstraints` bound lists accept NaN unguarded.~~** → fixed: `NumericClauseInterpreter.applyRealConstantBound` returns `false` for NaN values, so the clause is not marked consumed and stays residual (the filter enforces it). `RealConstraints` itself remains unguarded — it renders NaN as `Double.NaN` (compilable) for any direct caller — but the interpreter is the single entry point for SPF-derived bounds, and NaN never reaches the bound lists through it.
...
- **D-1 · PC→Model extraction can silently concretize/drop unsupported terms.** The listener + `SpfToModelTransformer` map SPF's constraint chain to the Model; unsupported SPF terms either throw (A-5) or, when a value is already concrete (raw bits under `z3`), are baked in as constants — which is exactly how the maxUlps spec collapsed to `0 < maxUlps`. **The dominant concretization surface is the native/MJI boundary, and it is detectable listener-only, no fork changes:** `JVMNativeStackFrame.setArguments` boxes concrete values and drops attrs (`jpf-core/.../JVMNativeStackFrame.java:36-125`); a peer preserves symbolics only via explicit `env.getArgAttributes()`/`setReturnAttribute` (e.g. `JPF_java_lang_Integer.valueOf`, `JPF_java_lang_Integer.java:80-88`). A Teralizer listener hook on `executeInstruction` for `EXECUTENATIVE` (`EXECUTENATIVE.getExecutedMethod()`) can check `StackFrame.getArgumentAttrs(mi)` (`StackFrame.java:962-990`) and record "symbolic value entered unmodeled native method" as a typed per-assertion event. Bytecode-executed (non-native) library code is NOT a concretization surface — attrs propagate through normal frames (`JVMStackFrame.java:59-67`) and symbolic branches collect constraints. Opportunity: the listener-side detector + a `concretization_events` count next to `output_spec_class` (`2026-07-01-pipeline-observability-telemetry`). Effort S–M (down from M — no jpf-symbc changes needed) · Sound? yes (silently incomplete spec → unsound assertTrue).
...
- **D-6 · `search.depth_limit=100` is hardcoded in `jpf-config.vm:51`.** In collect mode every symbolic-operand branch consumes a `PCChoiceGenerator` choice point (one per branch, `IFEQ.java:52-64`), so a loop over a symbolic value burns depth linearly — long-looping MUTs abort at 100. The abort IS typed (`ExtractionAborted.Reason.SEARCH_DEPTH_LIMIT`, `TestGeneralizationListener.java:76-78`) — not silent — but the value is unconfigurable (unlike `max-execution-time`/`max-path-condition-size`) and its firing frequency is unmeasured until the `task_diagnostic` rollups land. Opportunity: expose as `teralizer.jpf.max-search-depth` config + count `SEARCH_DEPTH_LIMIT` per project in `jpf_extraction_summary`. Effort S · Sound? no (recall only).
- **D-7 · `assertThrows` specs are extracted but the exception model is never rendered.** The listener captures thrown exceptions (type + message) with the PC (`TestGeneralizationListener.java:112-119,314-324`); `SpfToModelTransformer.transform(CapturedException)` produces an `ExceptionModel`; but `ModelToJavaTransformer.fold(ExceptionModel)` returns null (`ModelToJavaTransformer.java:368-370`) and `TestGeneralizationTask` only rewrites `RETURNED_VALUE` outputs (`TestGeneralizationTask.java:367`). Downside is small — the generated property keeps the original `assertThrows(E.class, …)` wrapper, which already oracles the exception *type* over generalized inputs — so the unused `ExceptionModel` costs only the message check. Opportunity: none urgent; note so nobody mistakes the null fold for a bug. Effort — · Sound? no.
...
1. ~~Quick correctness wins (S): A-2, A-4, B-2, C-5, D-2.~~ All fixed.

...
5. **Latent correctness bugs:** the `LCMP` long→int truncation (B-5) remains. The `MinMax` double lower-bound bug (B-2), the Error JSON round-trip (A-2), unquoted `ConstantString` (A-4), `Operator.get()` assert-only (D-2), and `RealConstraints` NaN-unguarded bounds (C-5) are fixed.

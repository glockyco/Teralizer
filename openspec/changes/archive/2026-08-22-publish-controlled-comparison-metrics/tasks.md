...
implementation, publication, and handoff tasks. They are prose records rather than incomplete tasks so
OpenSpec validation can distinguish canceled work from active work. This change was archived without
syncing its rejected delta specification.
...
- **Canceled:** 2.1 Add the approved mapping identity and qualification to the existing typed metric/provenance
      model only if task 1.9 approved that extension. Do not store the qualification only in prose.
- **Canceled:** 2.2 Implement separate read-only controlled and real-world adapters that emit the same typed
...
- **Canceled:** 2.3 Make one registered comparison owner declare both corpus inputs and invoke both adapters in one
...
- **Canceled:** 2.4 Emit numerator, denominator, and share metrics for each corpus with stable keys named after the
...
- **Canceled:** 2.5 Validate all metric relations and mapping invariants before report return. Publish no partial
      comparison when either side fails.

## 3. Defend mapping and publication behavior

- **Canceled:** 3.1 Add focused adapter tests for exact source-to-normalized mappings, denominator conservation,
      variant selection, eligibility, and source identity at both schema shapes.
- **Canceled:** 3.2 Add negative fixtures for every ambiguity found in task 1: competing lifecycle states, absent
...
- **Canceled:** 3.3 Prove the generated metric triplets agree with their adapters and, only where semantically
      equivalent, the existing RQ5 or RQ6 table row. Current count equality alone is not an assertion of
      mapping correctness.
- **Canceled:** 3.4 Extend rendering and manifest coverage to prove aggregate LaTeX macros and provenance retain
      both corpus identities, denominator keys, mapping identity, and qualification.
- **Canceled:** 3.5 Add a regression proving the comparison is labelled descriptive rather than paired or causal
      when project populations or eligibility differ.

## 4. Regenerate and verify producer outputs

- **Canceled:** 4.1 Run the focused mapping, RQ5, RQ6, metric-model, rendering, and manifest tests in the pinned Nix
      environment.
- **Canceled:** 4.2 Run the complete registered report set once through
...
- **Canceled:** 4.3 Update only normally generated, tracked producer artifacts required by the registered report
      run. Do not hand-edit a report, macro, provenance record, database, or corpus input.
- **Canceled:** 4.4 Run `uv run --directory analysis pytest`, `uv run --directory analysis ruff check .`, and
      `uv run --directory analysis ty check .` in the pinned Nix environment.
- **Canceled:** 4.5 Run `openspec validate publish-controlled-comparison-metrics --strict` and
      `lefthook run pre-commit --all-files`.

## 5. Commit and hand off the comparison contract

- **Canceled:** 5.1 Review the approved mapping record, implementation, negative controls, generated-output diff,
...
- **Canceled:** 5.2 Commit the implementation and generated comparison contract as one causal subject using
      `personal_commit`.
      Message: `feat(eval): publish mapped comparison metrics`
- **Canceled:** 5.3 Record the clean producer revision, normalized measure, both metric triplets and macro
